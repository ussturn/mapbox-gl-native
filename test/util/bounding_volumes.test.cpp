...
static util::Frustum createTestFrustum(double fovy, double aspectRatio, double zNear, double zFar, double elevation, double bearing) {
    mat4 proj;
    mat4 invProj;
...
TEST(BoundingVolumes, AabbFullyInsideFrustum) {
    ...
    const std::array<util::AABB, 3> aabbArray = {
        util::AABB({ -1, -1, 0 }, { 1, 1, 0 }),
        util::AABB({ -5, -5, 0 }, { 5, 5, 0 }),
        util::AABB({ -5, -5, 0 }, { -4, -2, 0 })
    };
